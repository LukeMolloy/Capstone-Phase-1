        ...
        <br /> <br />
            <?php
        		...
        		$userDependants = $_SESSION['ClientDependants'];
               	...
               	$sql="SELECT AES_DECRYPT(FirstName, 'show2016'), LastName, PhoneNumber, Address, Suburb, Postcode, City, State, Spacesavailable, Petfriendly, Gendertaken, Authorised, Nights FROM House WHERE Suburb LIKE '%$suburb%' AND PostCode LIKE '%$postcode%' AND City LIKE '%$city%' AND State LIKE '%$state%' AND SpacesAvailable >= $spaces AND InfantSpaces >= $userInfants AND PetFriendly >= $userPets";
